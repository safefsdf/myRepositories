...
public class FtpUtils {
	 ...
	 * @throws Exception
	 */
	public static String uploadFile(String partPath, byte[] decoderBytes,String imgName,String virtualPath) throws Exception {
		...
		try {
			...
			boolean changeWD = ftp.changeWorkingDirectory(partPath);// 转移到指定FTP服务器目录
			if (!changeWD) {
				if (!CreateDirecroty(partPath, ftp)) {
			...
			partPath = new String(partPath.getBytes("GBK"), "ISO-8859-1");
			// 转到指定上传目录
			ftp.changeWorkingDirectory(partPath);
			// 将上传文件存储到指定目录
	...
	private static void setArg() {
		try {
			userName = FtpCommons.userName;
			passWord = FtpCommons.passWord;
			hostName =FtpCommons.hostName;
			port = FtpCommons.port;
		} catch (Exception e1) {
			e1.printStackTrace();
		}
	}
