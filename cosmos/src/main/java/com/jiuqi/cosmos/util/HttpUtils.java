		...
		try {
			...
			if(response.getStatusLine().getStatusCode()==200) {
				...
				if(response.getEntity()!=null) {
					...
					//下载图片
					//声明outputstream OutputStreamWriter out = new OutputStreamWriter(new FileOutputStream(“dd.txt”),”UTF-8”);
					InputStream in = response.getEntity().getContent() ;
					String picName=null;
					int b ;
					picName=UUID.randomUUID().toString().replace("-", "")+suffix;
					OutputStream osOutputStream  = new FileOutputStream(new File("D:\\software\\123-JAVA\\temp\\"+picName));
					while((b=in.read()) != -1) {
